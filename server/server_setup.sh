...
git config --global user.email "lukeemersonwilley@gmail.com"
git config --global user.name "Luke Willey"

# Web app file structure
mkdir /apps
chmod 777 /apps
mkdir /apps/logs
mkdir /apps/logs/lukewilley
mkdir /apps/logs/lukewilley/app_log
cd /apps
...
cp /apps/lukewilley/server/lukewilley.service /etc/systemd/system/lukewilley.service

systemctl start lukewilley
systemctl status lukewilley
systemctl enable lukewilley
...
cp /apps/lukewilley/server/lukewilley.nginx /etc/nginx/sites-enabled/lukewilley.nginx
...
certbot --nginx -d lukewilley.com